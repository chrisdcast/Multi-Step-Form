...
export function Toggle({ params }: { params: IToggleParams }) {
    ...
    return (
        <div id={`${name} Toggle`} className={classNames ? `Toggle ${classNames.join('')}` : "Toggle"} onMouseDown={handleToggle}>
            <p className={'LabelLeft'}>{labelLeft}</p>
            <div className="ToggleFrame">
                <label htmlFor={name}></label>
                <input id={name} type="checkbox" {...register(name, registerParams)} value="true" />
